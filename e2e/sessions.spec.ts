...
test.describe('Session switching', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/v1/sessions/**/messages', async (route) => {
      ...
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify(messagesBySession[sessionId as keyof typeof messagesBySession] ?? []),
      })
    })
  ...
  test('renders the session list from API responses', async ({ page }) => {
    ...
    await expect(sessionSwitcher.getByRole('option', { name: /Planning Sync/i })).toBeVisible()
    await expect(sessionSwitcher.getByRole('option', { name: /Sprint Retro/i })).toBeVisible()
    await expect(sessionSwitcher.getByRole('option', { name: /Bug Bash Notes/i })).toBeVisible()
  ...
  test('displays session titles in the switcher', async ({ page }) => {
    ...
    await expect(sessionSwitcher.getByRole('button')).toContainText(/Planning Sync/)
  ...
  test('switches to the clicked session', async ({ page }) => {
    ...
    await expect(messageList).toContainText('Planning Sync summary from the API.')
    ...
    await expect(messageList).toContainText('Sprint Retro summary from the API.')
    await expect(messageList).not.toContainText('Planning Sync summary from the API.')
  })

  test('shows recent sessions in the session switcher dropdown', async ({ page }) => {
    const sessionSwitcher = page.getByTestId('session-switcher')

    await sessionSwitcher.getByRole('button').click()

    await expect(sessionSwitcher).toContainText('Recent Sessions')
    await expect(sessionSwitcher.getByRole('option', { name: /New Session/i })).toBeVisible()
