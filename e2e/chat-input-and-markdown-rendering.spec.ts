...
test.describe("Bug 5 — Shift+Enter inserts a newline in the composer", () => {
  ...
  test('typing "line one", Shift+Enter, "line two" produces "line one\\nline two" in the textarea and does not submit', async ({
    ...
    page.on("request", (request) => {
      if (
        request.method() === "POST" &&
        /\/sessions\/[^/]+\/messages$/.test(request.url())
      ) {
        postedSendCount += 1;
      }
    });
...
test.describe("Bug 3 — Markdown renders as HTML in assistant bubbles", () => {
  ...
  test("headings, lists, code, bold/italic, link, table, and blockquote all render as HTML elements inside the assistant bubble", async ({
    page,
  }) => {
    ...
    // Blockquote.
    await expect(
      md.locator("blockquote", { hasText: "a blockquote line" }),
    ).toHaveCount(1);
