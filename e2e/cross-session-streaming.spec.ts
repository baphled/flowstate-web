...
test.describe("Cross-session streaming — chunks for session A must not land on session B", () => {
  test("switching sessions mid-stream does not contaminate the new session with the old session's chunks", async ({
    page,
  }) => {
    ...
    > = {
      "session-A": [
        {
          id: "a-u0",
          role: "user",
          content: "A prior prompt",
          timestamp: "2026-05-04T00:00:00Z",
        },
        {
          id: "a-a0",
          role: "assistant",
          content: "A prior reply (completed)",
      ...
      "session-B": [
        ...
        },
        {
          id: "b-a1",
          role: "assistant",
          content: "B canonical reply (must not be corrupted)",
          timestamp: "2026-05-04T00:01:01Z",
        },
      ],
    };
    ...
    await page.route("**/api/v1/sessions/*/messages", async (route) => {
      ...
      const sessionId =
        url.match(/\/sessions\/([^/]+)\/messages/)?.[1] ?? "session-A";
    ...
    await expect(bAssistant).toContainText(
      "B canonical reply (must not be corrupted)",
    );
