  ...
  await page.route('**/api/v1/sessions/*/messages', async (route) => {
    ...
    if (route.request().method() === 'POST') {
      ...
      messagesBySession[sessionId] = [
        ...messagesBySession[sessionId],
        { id: `srv-u${Date.now()}`, role: 'user', content: body.content ?? '', timestamp: new Date().toISOString() },
      ]
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
...
test.describe('Stuck session after reload — SSE reconnect + visible submit gate', () => {
  test('post-reload SSE chunks reach the in-flight assistant message', async ({ page }) => {
  ...
  test('typing while a send is still pending surfaces a toast — no silent drop', async ({ page }) => {
    await installFakeSSE(page)
    const gate = newGate()
    await setupBackendMocks(page, gate)
    ...
    const toast = page.getByTestId('toast-container').getByTestId('toast-item').first()
