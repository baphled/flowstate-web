...
async function installRoutes(page: Page, messages: typeof placeholderMessages): Promise<void> {
...
test.describe('Thinking-only degraded turn — live UI', () => {
  test('the synthesised placeholder renders as a soft-error affordance, not a blank bubble', async ({ page }) => {
  ...
  test('a normal content-bearing assistant message does NOT trigger the affordance', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="thinking-only-affordance"]')).toHaveCount(0)
  })
})
