...
async function bootstrapMocks(page: Page): Promise<void> {
  const messages = [
    { id: 's1-u', role: 'user', content: 'hello', timestamp: '2026-05-07T00:00:00Z' },
    {
      id: 's1-a',
      role: 'assistant',
      content: 'world',
...
async function gotoChatAndOpenStream(page: Page): Promise<SSEChannel> {
  ...
  return page.evaluateHandle(() => {
    const w = window as unknown as { __sseDriver: { instances: () => SSEChannel[] } }
...
async function fireSSE(page: Page, payload: object | string): Promise<void> {
  await page.evaluate((data) => {
    const w = window as unknown as { __sseDriver: { instances: () => Array<{ fire: (t: string, d: unknown) => void }> } }
...
const DEV_BASE_URL = process.env['CRITICAL_BANNER_BASE_URL'] ?? 'http://localhost:5173'
...
test.describe('Critical error banner — live UI', () => {
  ...
  test('banner is hidden before any event and renders on a stream_critical chunk', async ({ page }) => {
    ...
    await fireSSE(page, JSON.stringify({ error: 'critical stream error', correlation_id: 'live-id-1' }))

    const banner = page.locator('[data-testid="critical-error-banner"]')
    await expect(banner).toBeVisible()
    await expect(banner).toHaveAttribute('role', 'alert')
    await expect(page.locator('[data-testid="critical-error-message"]')).toContainText(
      'critical stream error',
    )
  })

  test('Show details reveals the correlation id and Dismiss clears the banner', async ({ page }) => {
    await gotoChatAndOpenStream(page)
    await fireSSE(page, JSON.stringify({ error: 'critical stream error', correlation_id: 'live-id-2' }))

    await expect(page.locator('[data-testid="critical-error-banner"]')).toBeVisible()
    await expect(page.locator('[data-testid="critical-error-correlation-id"]')).toHaveCount(0)

    await page.locator('[data-testid="critical-error-details-toggle"]').click()
    await expect(page.locator('[data-testid="critical-error-correlation-id"]')).toHaveText(
      'live-id-2',
    )
  ...
  test('a fresh stream_critical event re-shows the banner after dismissal with the new id', async ({ page }) => {
    await gotoChatAndOpenStream(page)

    await fireSSE(page, JSON.stringify({ error: 'critical stream error', correlation_id: 'first-live-id' }))
    await page.locator('[data-testid="critical-error-dismiss"]').click()
    await expect(page.locator('[data-testid="critical-error-banner"]')).toHaveCount(0)

    await fireSSE(page, JSON.stringify({ error: 'critical stream error', correlation_id: 'second-live-id' }))
  ...
  test('a transient stream_error event does NOT render the critical banner', async ({ page }) => {
    await gotoChatAndOpenStream(page)

    await fireSSE(page, JSON.stringify({ error: 'stream error', correlation_id: 'transient-id' }))

    await expect(page.locator('[data-testid="critical-error-banner"]')).toHaveCount(0)
  })

  test('banner uses the red severity palette (computed background)', async ({ page }) => {
    await gotoChatAndOpenStream(page)
    await fireSSE(page, JSON.stringify({ error: 'critical stream error', correlation_id: 'visual-id' }))
