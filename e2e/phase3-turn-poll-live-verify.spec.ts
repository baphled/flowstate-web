...
test.describe('Phase 3 — turn-poll live render', () => {
  ...
  test('polls fetchTurn and renders assistant content without refresh', async ({ page }) => {
    ...
    page.on('request', (req) => {
      ...
      if (url.match(/\/api\/v1\/sessions\/[^/]+\/messages$/) && method === 'POST') {
        networkEvents.push({ t: Date.now() - startedAt, kind: 'POST messages', detail: url })
      } else if (url.match(/\/api\/v1\/sessions\/[^/]+\/turns\/[^/?]+/)) {
        networkEvents.push({ t: Date.now() - startedAt, kind: 'GET turn', detail: url })
      } else if (url.match(/\/api\/v1\/sessions\/[^/]+\/stream/)) {
        networkEvents.push({ t: Date.now() - startedAt, kind: 'SSE stream', detail: url })
      }
    })
    ...
    const bubbleCount = await page.locator('.message-bubble.assistant, .message-bubble.thinking').count()
    let lastBubbleText: string | null = null
    if (bubbleCount > 0) {
      lastBubbleText = await page.locator('.message-bubble.assistant, .message-bubble.thinking').last().textContent()
    }

    const postCount = networkEvents.filter((e) => e.kind === 'POST messages').length
    const turnPollCount = networkEvents.filter((e) => e.kind === 'GET turn').length
    const sseCount = networkEvents.filter((e) => e.kind === 'SSE stream').length

    // eslint-disable-next-line no-console
    console.log('NETWORK SUMMARY:', { postCount, turnPollCount, sseCount })
    // eslint-disable-next-line no-console
    console.log('DOM SUMMARY:', { bubbleVisible, bubbleCount, lastBubbleText: lastBubbleText?.slice(0, 200) })
    // eslint-disable-next-line no-console
    console.log('TIMELINE:', JSON.stringify(networkEvents.slice(0, 30).map((e) => ({ t: e.t, kind: e.kind }))))

    // Phase 3 assertions.
    expect(postCount, 'POST /messages must fire').toBeGreaterThanOrEqual(1)
    expect(turnPollCount, 'GET /turns/{turn_id} must fire at least twice').toBeGreaterThanOrEqual(2)
    expect(bubbleVisible, 'assistant/thinking bubble rendered live (no refresh)').toBe(true)
    expect(lastBubbleText?.length ?? 0, 'bubble content rendered without refresh').toBeGreaterThan(0)
  })
})
