...
async function bootstrapMocks(page: Page): Promise<void> {
  const messages = [
    { id: 's1-u', role: 'user', content: 'hello', timestamp: '2026-05-08T00:00:00Z' },
    {
      id: 's1-a',
      role: 'assistant',
      content: 'world',
...
const DEV_BASE_URL = process.env['CONTEXT_USAGE_BASE_URL'] ?? 'http://localhost:5173'
...
test.describe('Context usage chip — live UI', () => {
  ...
  test('chip pivots to warning severity at 75%+ and danger at 90%+', async ({ page }) => {
    await gotoChatAndOpenStream(page)
    ...
    const colour = await chip.evaluate((el) => window.getComputedStyle(el).color)
  ...
  test('chip updates the rendered figure as successive context_usage chunks arrive', async ({
    ...
    await expect(page.locator('[data-testid="context-usage-percentage"]')).toContainText('1%')
    ...
    await expect(page.locator('[data-testid="context-usage-percentage"]')).toContainText('92%')
    await expect(page.locator('[data-testid="context-usage-chip"]')).toHaveAttribute(
      'data-severity',
      'danger',
    )
  })
