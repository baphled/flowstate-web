...
async function recordSnapshot(page: Page, phase: string, snapshots: DomSnapshot[], evidenceDir: string): Promise<DomSnapshot> {
  const snap = await snapshot(page, phase)
  snapshots.push(snap)
  await page.screenshot({ path: path.join(evidenceDir, `${phase}.png`), fullPage: true })
  return snap
}
...
test.describe('manual session drive — real backend evidence capture', () => {
  ...
  test('two turns, full evidence capture', async ({ page }) => {
    ...
    page.on('response', async (res: Response) => {
      ...
      try {
        ...
        if (!ct.includes('event-stream')) {
          ...
          responseBodyPreviews[`${nowIso()}-${url}`] = buf.toString('utf-8').slice(0, 5000)
        } else {
          size = 0
    ...
    await expect(page.getByTestId('agent-picker')).toContainText(/senior/i, { timeout: 10_000 })
    ...
    await recordSnapshot(page, '02-turn1-post-click-250ms', snapshots, EVIDENCE_DIR)

    await page.waitForTimeout(750) // ~1s after click
    await recordSnapshot(page, '03-turn1-post-click-1s', snapshots, EVIDENCE_DIR)

    await page.waitForTimeout(2000) // ~3s after click
    await recordSnapshot(page, '04-turn1-post-click-3s', snapshots, EVIDENCE_DIR)
    ...
    try {
      ...
      console.error(`[evidence] turn 1: no assistant bubble in 90s — capturing failure state`)
      await recordSnapshot(page, '05-turn1-FAILURE-no-assistant', snapshots, EVIDENCE_DIR)
      throw e
    }

    // Wait for the indicator to settle (hide)
    try {
      await expect(page.getByTestId('agent-activity-indicator')).toBeHidden({ timeout: 30_000 })
    } catch {
      console.warn('[evidence] turn 1: indicator did not hide within 30s — capturing stuck state')
      await recordSnapshot(page, '06-turn1-indicator-stuck', snapshots, EVIDENCE_DIR)
    }
    ...
    fs.writeFileSync(path.join(EVIDENCE_DIR, 'turn1-click-ts.txt'), turn1ClickTs)
    ...
    await page.waitForTimeout(250)
    await recordSnapshot(page, '09-turn2-post-click-250ms', snapshots, EVIDENCE_DIR)

    await page.waitForTimeout(750)
    await recordSnapshot(page, '10-turn2-post-click-1s', snapshots, EVIDENCE_DIR)

    await page.waitForTimeout(2000)
    await recordSnapshot(page, '11-turn2-post-click-3s', snapshots, EVIDENCE_DIR)

    try {
      await expect
        .poll(
          async () => await page.locator('.message-bubble.assistant').count(),
          { timeout: 90_000, message: 'expected ≥2 assistant bubbles after turn 2' },
        )
        .toBeGreaterThanOrEqual(2)
    } catch (e) {
      console.error('[evidence] turn 2: <2 assistant bubbles after 90s')
      await recordSnapshot(page, '12-turn2-FAILURE-no-second-assistant', snapshots, EVIDENCE_DIR)
      throw e
    }
    ...
    try {
      ...
      await recordSnapshot(page, '13-turn2-indicator-stuck', snapshots, EVIDENCE_DIR)
    }

    await page.waitForTimeout(2_000)
    await recordSnapshot(page, '14-turn2-settled', snapshots, EVIDENCE_DIR)

    fs.writeFileSync(path.join(EVIDENCE_DIR, 'turn2-click-ts.txt'), turn2ClickTs)

    // === DUMP EVIDENCE ===
    fs.writeFileSync(path.join(EVIDENCE_DIR, 'network.json'), JSON.stringify(network, null, 2))
    fs.writeFileSync(path.join(EVIDENCE_DIR, 'console.json'), JSON.stringify(consoleLog, null, 2))
    ...
    try {
      const canonRes = await page.request.get(`http://localhost:8080/api/v1/sessions/${sessionId}/messages`)
      const canonText = await canonRes.text()
      fs.writeFileSync(path.join(EVIDENCE_DIR, 'canonical-messages.json'), canonText)
    } catch (e) {
      fs.writeFileSync(path.join(EVIDENCE_DIR, 'canonical-messages.error'), (e as Error).message)
    }

    console.log(`[evidence] captured ${snapshots.length} snapshots, ${network.length} net events, ${consoleLog.length} console msgs`)
