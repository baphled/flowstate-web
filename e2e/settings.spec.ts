...
test.describe('Settings view', () => {
  ...
  test('selects dark theme and updates data-theme on html', async ({ page }) => {
    await page.getByTestId('theme-option-dark').click()
    const htmlTheme = await page.evaluate(() => document.documentElement.getAttribute('data-theme'))
    expect(htmlTheme).toBe('dark')
  })

  test('selects light theme and updates data-theme on html', async ({ page }) => {
    await page.getByTestId('theme-option-light').click()
    const htmlTheme = await page.evaluate(() => document.documentElement.getAttribute('data-theme'))
    expect(htmlTheme).toBe('light')
  })

  test('selects terminal theme and updates data-theme on html', async ({ page }) => {
