  ...
  await page.addInitScript(() => {
    ...
    class FakeEventSource {
      ...
      constructor(url: string) {
        this.url = url
        ;(w.__sseInstances as FakeEventSource[] | undefined)?.push(this) ?? (w.__sseInstances = [this])
      }
  ...
  await page.route('**/api/v1/sessions/*/messages', async (route) => {
    ...
    if (route.request().method() === 'POST') {
      ...
      messagesBySession[sessionId] = [
        ...
        { id: assistantId, role: 'assistant', content: responseText, timestamp: new Date().toISOString() },
      ]
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
...
test.describe('Multi-turn streaming — second turn must not overwrite first', () => {
  test('two consecutive turns render two distinct assistant bubbles, even after refetch', async ({ page }) => {
    ...
    await expect(page.getByTestId('message-assistant').nth(0)).toContainText(TURN_1_RESPONSE)
    ...
    await page.waitForFunction(() => {
      return (window as unknown as { __sseDriver?: { instances: () => unknown[] } }).__sseDriver?.instances().length === 2
    })
    ...
    await expect(page.getByTestId('message-assistant')).toHaveCount(2)
    await expect(page.getByTestId('message-assistant').nth(0)).toContainText(TURN_1_RESPONSE)
    await expect(page.getByTestId('message-assistant').nth(0)).not.toContainText(TURN_2_RESPONSE)
    await expect(page.getByTestId('message-assistant').nth(1)).toContainText(TURN_2_RESPONSE)
  })
})
