...
async function installCommonRoutes(page: Page, opts: RoutesOptions = {}): Promise<void> {
  ...
  await page.route('**/api/v1/sessions/*/messages', async (route: Route) => {
    ...
    const sessionId = url.match(/\/sessions\/([^/]+)\/messages/)?.[1] ?? BASE_SESSION.id
    if (route.request().method() === 'POST') {
...
async function waitForSSE(page: Page): Promise<void> {
  await page.waitForFunction(() => {
    return (
      ...
      ).__sseDriver?.instances().length ?? 0
    ) >= 1
  })
}
...
test.describe('Composer parity — PR2 + bug fixes', () => {
  ...
  test('Stop button replaces Send during stream and cancels on click', async ({ page }) => {
    const gate = newGate()
  ...
  test('ArrowUp recalls per-session prompt history in LIFO order', async ({ page }) => {
  ...
  test('? opens KeyboardHelpModal only when no input/button is focused', async ({ page }) => {
    await installCommonRoutes(page)
    await page.goto('/chat')
    await expect(page.getByTestId('message-input')).toBeVisible()
  ...
  test('Attachment picker renders and drag overlay surfaces on file drag', async ({ page }) => {
    await installCommonRoutes(page)
    await page.goto('/chat')
    ...
    await expect(page.getByTestId('message-input-drag-overlay')).toBeVisible({ timeout: 5_000 })
    ...
    await page.evaluate(() => {
      ...
      const dt = new DataTransfer()
      dt.items.add(new File(['x'], 'pixel.png', { type: 'image/png' }))
      wrap.dispatchEvent(new DragEvent('dragleave', { dataTransfer: dt, bubbles: true }))
    })
  ...
  test('Send with attachment + empty text surfaces a toast and does not POST', async ({ page }) => {
    await installCommonRoutes(page)
    await page.goto('/chat')

    let postedSendCount = 0
    page.on('request', (req) => {
      if (req.method() === 'POST' && /\/sessions\/[^/]+\/messages$/.test(req.url())) {
        postedSendCount += 1
      }
    })
...
test.describe('Code rendering parity — PR1', () => {
  ...
  test('Fenced JS block renders Shiki tokens, not plain text', async ({ page }) => {
    const code = "const greet = (name) => `hello ${name}`"
    const reply = `Here is a snippet:\n\n\`\`\`js\n${code}\n\`\`\`\n`
    await installCommonRoutes(page, {
      onPost: () => [
        { id: 'a1', role: 'assistant', content: reply, timestamp: new Date().toISOString() },
      ],
    })
    await page.goto('/chat')
    ...
    // theme contract.
    const styledSpans = shikiPre.locator('span[style*="--shiki"], span[style*="color:"]')
    await expect.poll(() => styledSpans.count(), { timeout: 5_000 }).toBeGreaterThan(0)
  })

  // [7] ThinkingPanel — collapsible <details> element, MarkdownRenderer
  // inside renders fenced code with Shiki.
  test('ThinkingPanel renders as <details> with markdown body', async ({ page }) => {
  ...
  test('Each fenced code block emits a per-block copy button', async ({ page }) => {
    const reply = [
      'First snippet:',
      '',
      '```js',
...
test.describe('Tool cards parity — PR3 + bug fixes', () => {
  ...
  // wants to inspect.
  test('Bash and Read tool cards default-collapsed; Edit defaults open', async ({ page }) => {
    const gate = newGate()
    await installCommonRoutes(page, { postGate: gate })
    await page.goto('/chat')
    ...
    for (const tool of ['bash', 'read', 'edit']) {
      await fireSSE(page, 'message', { type: 'tool_call', name: tool, status: 'running' })
      await fireSSE(page, 'message', {
        type: 'tool_result',
        content: tool === 'edit' ? '@@ -1,1 +1,1 @@\n-old\n+new\n' : `${tool} output`,
      })
    }

    const bash = page.locator('[data-testid="tool-renderer"][data-tool="bash"]').first()
    const read = page.locator('[data-testid="tool-renderer"][data-tool="read"]').first()
    const edit = page.locator('[data-testid="tool-renderer"][data-tool="edit"]').first()
  ...
  test('Regenerate button shows on completed assistant; hidden during stream', async ({ page }) => {
    const gate = newGate()
    await installCommonRoutes(page, {
      postGate: gate,
      onPost: () => [
        { id: 'srv-u-0', role: 'user', content: 'hi', timestamp: '2026-05-10T00:00:00Z' },
        { id: 'srv-a-0', role: 'assistant', content: 'hello there', timestamp: '2026-05-10T00:00:01Z' },
      ],
    })
    await page.goto('/chat')
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            ...
            messages: [
              { id: 'srv-u-0', role: 'user', content: 'hi', timestamp: '2026-05-10T00:00:00Z' },
              { id: 'srv-a-0', role: 'assistant', content: 'hello there', timestamp: '2026-05-10T00:00:01Z' },
              { id: 'srv-u-1', role: 'user', content: 'again', timestamp: '2026-05-10T00:00:02Z' },
              { id: 'srv-a-1', role: 'assistant', content: 'another reply', timestamp: '2026-05-10T00:00:03Z' },
            ],
          }),
        })
        return
...
test.describe('Theme polish — PR4', () => {
  ...
  test('Selecting Tokyo Night sets <html data-theme="tokyo-night">', async ({ page }) => {
  ...
  test('Hovering a theme option live-previews; leaving reverts', async ({ page }) => {
    await page.goto('/settings')
    await expect(page.getByTestId('settings-view')).toBeVisible()
...
test.describe('Tier-3 polish — PR6', () => {
  ...
  test('EditTool renders one hunk block per @@ marker', async ({ page }) => {
    ...
    await expect(page.getByTestId('edit-hunk-header').first()).toContainText('@@ -1,3 +1,3 @@')
  })
  ...
  test('Recall results render relative timestamp + chain-depth chip', async ({ page }) => {
    ...
    await installCommonRoutes(page, { postGate: gate })
    await page.goto('/chat')
    await page.getByTestId('message-input').fill('what did I say about retries')
    await page.getByTestId('send-button').click()
    await waitForSSE(page)
    ...
    await fireSSE(page, 'message', { type: 'tool_result', content: recallBody })

    const card = page.locator('[data-testid="tool-renderer"][data-tool="search_context"]').first()
  ...
  test('LoadingOverlay reveals when bootstrap is slow (>200ms gate)', async ({ page }) => {
    await installFakeEventSource(page)
    ...
    await expect(page.getByTestId('app-loading-overlay')).toBeVisible({ timeout: 1_500 })
  ...
  test('Collapse-all and Expand-all toolbar buttons bulk-flip tool cards', async ({ page }) => {
    const gate = newGate()
    await installCommonRoutes(page, { postGate: gate })
    await page.goto('/chat')
...
test.describe('Live token counter — PR5', () => {
  ...
  test('Token counter renders cumulative count + rate during stream', async ({ page }) => {
    const gate = newGate()
    await installCommonRoutes(page, { postGate: gate })
    await page.goto('/chat')
  ...
  test('Token counter disappears after stream completes', async ({ page }) => {
    ...
    await installCommonRoutes(page, {
      postGate: gate,
      onPost: () => [
        { id: 'srv-u-0', role: 'user', content: 'done?', timestamp: '2026-05-12T00:00:00Z' },
        { id: 'srv-a-0', role: 'assistant', content: 'done.', timestamp: '2026-05-12T00:00:01Z' },
      ],
    })
    await page.goto('/chat')
