...
async function installCommonRoutes(page: Page): Promise<void> {
  ...
  await page.route("**/api/v1/sessions/**/messages", async (route: Route) => {
    ...
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify([{ role: "assistant", content: ASSISTANT_REPLY }]),
    });
  });
...
test.describe("Chat Attachments Backend (May 2026) — PR2 task-08 cross-session injection (R9)", () => {
  test("drops cross-session <img> AND fires attachment_blocked.cross_session", async ({
    ...
    await page.addInitScript(() => {
      (
        window as unknown as { __crossSessionBlocks: string[] }
      ).__crossSessionBlocks = [];
      window.addEventListener(
        "attachment_blocked.cross_session",
        (e: Event) => {
          ...
          (
            window as unknown as { __crossSessionBlocks: string[] }
          ).__crossSessionBlocks.push(detail?.src ?? "");
        },
      );
    });
    ...
    expect(
      blocks.some(
        (s) => s.includes("session-OTHER") && s.includes("aid-stolen"),
      ),
    ).toBe(true);
  });
});
