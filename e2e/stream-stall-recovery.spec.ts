  ...
  await page.route('**/api/v1/sessions/*/messages', async (route) => {
    ...
    if (route.request().method() === 'POST') {
      ...
      messagesBySession[sessionId] = [
        { id: 'srv-u1', role: 'user', content: 'long task', timestamp: '2026-05-04T00:00:00Z' },
        { id: 'srv-a1', role: 'assistant', content: canonicalReply, timestamp: '2026-05-04T00:00:02Z' },
      ]
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
...
test.describe('Stream stall recovery — watchdog trip reconciles to canonical', () => {
  test('a stalled SSE recovers to backend canonical state when the watchdog trips', async ({ page }) => {
    ...
    const gate = newGate()
    const canonicalReply = 'recovered: the canonical completed reply backend persisted'
    ...
    await expect(lastAssistant).toContainText(canonicalReply, { timeout: 5000 })
  })
})
