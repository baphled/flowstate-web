...
test.describe("Session hierarchy navigation", () => {
  ...
  test("keeps the input-selector-bar visible in a child session in read-only mode and surfaces the model/provider used", async ({
    page,
  }) => {
    // Parent session: bar visible, pickers interactive.
    const bar = page.getByTestId("input-selector-bar");
    await expect(bar).toBeVisible();
    await expect(bar.getByTestId("agent-picker")).not.toHaveClass(
      /is-readonly/,
    );
    await expect(bar.getByTestId("model-picker")).not.toHaveClass(
      /is-readonly/,
    );
    ...
    await expect(bar.getByTestId("toolbar-provider-label")).toContainText(
      "ollama",
    );
  ...
  test("clicking a delegation card loads the child session directly", async ({
    page,
  }) => {
    const strip = page.getByTestId("delegation-strip");
  ...
  test("Ctrl+X then ArrowDown loads the most-recent child (child-c)", async ({
    page,
  }) => {
  ...
  test("ArrowLeft and ArrowRight navigate siblings inside a child session", async ({
    page,
  }) => {
    // Start by entering child-b via the delegation strip.
    await page
      .getByTestId("delegation-strip")
  ...
  test("ArrowUp on a child session navigates to the parent", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByTestId("input-selector-bar").getByTestId("agent-picker"),
