...
import { recordStreamEvent, getStreamLog, clearStreamLog, STREAM_LOG_MAX } from './streamLog'
...
describe('streamLog', () => {
  ...
  afterEach(() => {
    ...
    if (typeof window !== 'undefined') {
      delete (window as unknown as { __flowstateStreamLog?: unknown }).__flowstateStreamLog
    }
  })
  ...
  it('mirrors the live ring onto window.__flowstateStreamLog so it can be inspected from DevTools', () => {
    ...
    recordStreamEvent({ kind: 'reconcile-result', sessionId: 'session-1', messageCount: 3 })

    const w = window as unknown as { __flowstateStreamLog?: ReturnType<typeof getStreamLog> }
  ...
  it('records all the lifecycle kinds we care about (connect/disconnect/watchdog-arm/watchdog-trip/watchdog-clear/reconcile-call/reconcile-result)', () => {
    ...
    recordStreamEvent({ kind: 'reconcile-result', sessionId: 's', messageCount: 7 })
