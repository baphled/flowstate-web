...
describe("isAllowedApiHost — default policy", () => {
  ...
  it("accepts https:// same-origin URLs", () => {
    expect(isAllowedApiHost("https://app.flowstate.example/api", opts)).toBe(
      true,
    );
  });

  it("rejects https:// different-origin URLs by default", () => {
    expect(isAllowedApiHost("https://api.flowstate.example/v1", opts)).toBe(
      false,
    );
  ...
  it("accepts extra allowed origins when supplied (operator opt-in)", () => {
    ...
    expect(isAllowedApiHost("https://api.flowstate.example/v1", trusting)).toBe(
      true,
    );
...
describe("validateApiHost", () => {
  ...
  it("returns the value unchanged when permitted", () => {
    ...
    expect(validateApiHost("http://localhost:8080", opts)).toBe(
      "http://localhost:8080",
    );
  });
