  ...
  <div class="settings-view" data-testid="settings-view">
    ...
    <section class="settings-section" data-testid="api-section">
      ...
      <input
        ...
        @input="
          settingsStore.setApiHost(($event.target as HTMLInputElement).value)
        "
        placeholder="http://localhost:8080"
      />
      <p class="field-hint">
        Base URL for the FlowState Go server. Vite proxies <code>/api</code> in
        dev mode.
      </p>
    </section>
    ...
    >
      ...
      <p class="field-hint">
        ...
        fraction of the model's context limit. Lower values compact sooner (more
        aggressive); higher values compact later (more headroom for long
        sessions before summary). 0.75 is the historical default.
      </p>
      <p
        v-if="thresholdError"
        class="field-error"
        data-testid="compression-threshold-error"
      >
        {{ thresholdError }}
      </p>
    </section>
