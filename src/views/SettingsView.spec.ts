...
vi.mock("@/api", async (importOriginal) => {
  ...
  return {
    ...
    updateCompressionThreshold: vi
      .fn()
      .mockImplementation((t: number) => Promise.resolve({ threshold: t })),
  };
});
...
describe("SettingsView - compression threshold control", () => {
  ...
  it("renders a compression section once the backend reports a configured threshold", async () => {
    ...
    expect(wrapper.find('[data-testid="compression-section"]').exists()).toBe(
      true,
    );
    expect(
      wrapper.find('[data-testid="compression-threshold-input"]').exists(),
    ).toBe(true);
  });

  it("hydrates the threshold input from the backend response", async () => {
    const { fetchCompressionConfig } = await import("@/api");
    vi.mocked(fetchCompressionConfig).mockResolvedValueOnce({
      threshold: 0.42,
    });
...
describe("SettingsView - theme picker (N2 + N1)", () => {
  ...
  it("renders all additional theme options (N2)", async () => {
    ...
    expect(wrapper.find('[data-testid="theme-option-dark"]').exists()).toBe(
      true,
    );
    expect(wrapper.find('[data-testid="theme-option-light"]').exists()).toBe(
      true,
    );
    expect(wrapper.find('[data-testid="theme-option-terminal"]').exists()).toBe(
      true,
    );

    // N2 additions.
    expect(
      wrapper.find('[data-testid="theme-option-tokyo-night"]').exists(),
    ).toBe(true);
    expect(
      wrapper.find('[data-testid="theme-option-catppuccin-mocha"]').exists(),
    ).toBe(true);
    expect(wrapper.find('[data-testid="theme-option-dracula"]').exists()).toBe(
      true,
    );
    expect(wrapper.find('[data-testid="theme-option-nord"]').exists()).toBe(
      true,
    );
  });
  ...
  it("does not override an explicit selection on mouseleave (N1)", async () => {
    ...
    const nordRadio = wrapper.find(
      '[data-testid="theme-option-nord"] input[type="radio"]',
    );
