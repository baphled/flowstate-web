...
describe('ProviderQuotaDashboardView', () => {
  ...
  it('renders the empty state when the aggregator returns []', async () => {
    ...
    expect(wrapper.find('[data-testid="provider-quota-dashboard-empty"]').exists()).toBe(true)
    expect(wrapper.find('[data-testid="provider-quota-dashboard-table"]').exists()).toBe(false)
  })
  ...
  it('renders the feature-off banner when the server reports 501', async () => {
    ...
    expect(
      wrapper.find('[data-testid="provider-quota-dashboard-feature-off"]').exists(),
    ).toBe(true)
  })
  ...
  it('surfaces a 401 from the backend as an error message (uniform B8 carry-through)', async () => {
    ...
    expect(wrapper.find('[data-testid="provider-quota-dashboard-error"]').exists()).toBe(true)
  })

  it('renders one row per entry with provider, truncated account, and model', async () => {
    fetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify([rateLimitRow, tokenSpendRow, notConfiguredRow]), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      }),
    )
    ...
    expect(rateRow.find('[data-testid="provider-quota-row-account"]').text()).toBe('abc12345')
    expect(rateRow.find('[data-testid="provider-quota-row-variant"]').text()).toBe(
      'Rate-limit',
    )
    expect(rateRow.find('[data-testid="provider-quota-row-summary"]').text()).toContain(
      '12% remaining',
    )

    // Token-spend row.
    const spendRow = wrapper.find('[data-testid="provider-quota-row-openai-def67890def-gpt-4o"]')
    expect(spendRow.exists()).toBe(true)
    expect(spendRow.find('[data-testid="provider-quota-row-summary"]').text()).toContain('12.34')

    // Not-configured row with empty account_hash falls back to "(default)".
    const naRow = wrapper.find('[data-testid="provider-quota-row-ollama--llama3:8b"]')
    expect(naRow.exists()).toBe(true)
    expect(naRow.find('[data-testid="provider-quota-row-account"]').text()).toBe('(default)')
    expect(naRow.find('[data-testid="provider-quota-row-summary"]').text()).toContain(
  ...
  it('opens the ProviderQuotaPanel modal when a row is clicked', async () => {
    ...
    expect(wrapper.find('[data-testid="provider-quota-panel"]').exists()).toBe(false)

    const row = wrapper.find('[data-testid="provider-quota-row-openai-def67890def-gpt-4o"]')
    await row.trigger('click')
    await flushPromises()

    expect(wrapper.find('[data-testid="provider-quota-panel"]').exists()).toBe(true)
    expect(wrapper.find('[data-testid="provider-quota-panel-model"]').text()).toBe('gpt-4o')
  })
})
