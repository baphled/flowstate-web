...
describe('buildToolRenderSpec', () => {
  ...
  it('renders preferred fallback keys for tools outside the allowlist', () => {
    ...
    expect(
      buildToolRenderSpec(
        makeToolMessage('tool_call', 'search_nodes', { query: 'FlowState recall', limit: 10 }),
      ).heading,
    ...
    expect(
      buildToolRenderSpec(makeToolMessage('tool_call', 'background_output', { id: 'bg-task-42' }))
        .heading,
    ).toBe('background_output bg-task-42')
  })

  it('falls back to a deterministic compact-JSON object when no preferred key matches', () => {
    const msg = makeToolMessage('tool_call', 'mystery_tool', { alpha: 'first', zeta: 'last' })
    // Keys are sorted to keep the rendered heading stable across reloads.
    expect(buildToolRenderSpec(msg).heading).toBe('mystery_tool {"alpha":"first","zeta":"last"}')
  })

  it('falls back to the bare tool name when args contains only non-string values', () => {
    const msg = makeToolMessage('tool_call', 'foo_bar', { count: 5, enabled: true })
  ...
  it('truncates long fallback values at 80 characters with an ellipsis', () => {
    ...
    const msg = makeToolMessage('tool_call', 'search_nodes', { query: longQuery })
    expect(buildToolRenderSpec(msg).heading).toBe('search_nodes ' + 'a'.repeat(80) + '...')
  })
