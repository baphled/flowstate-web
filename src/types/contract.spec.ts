...
describe("Provider Quota wire contract — Go ↔ TS hand-mirror", () => {
  ...
  it("sseProviderQuota top-level fields parse from a representative payload", () => {
    ...
    for (const required of goProviderQuotaFields) {
      ...
      expect(
        Object.prototype.hasOwnProperty.call(wire, required) ||
          wire[required] === undefined,
      ).toBe(true);
    }
  ...
  it("NotConfigured variant carries the operator-visible Reason verbatim", () => {
    ...
    // recognised pattern.
    expect(
      "unknown-model:claude-experimental".startsWith("unknown-model:"),
    ).toBe(true);
  });
  ...
  it("exactly-one-of variant discriminant — invariant pin", () => {
    ...
    const rateLimitWire = {
      variant: "rate_limit",
      rate_limit: {},
      token_spend: undefined,
      not_configured: undefined,
    };
    const tokenSpendWire = {
      variant: "token_spend",
      rate_limit: undefined,
      token_spend: {},
      not_configured: undefined,
    };
    const notConfiguredWire = {
    ...
    for (const wire of [rateLimitWire, tokenSpendWire, notConfiguredWire]) {
      const populated = (
        ["rate_limit", "token_spend", "not_configured"] as const
      ).filter((k) => (wire as Record<string, unknown>)[k] !== undefined);
