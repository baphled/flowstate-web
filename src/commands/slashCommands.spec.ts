...
describe("SLASH_COMMANDS", () => {
  it("mirrors the TUI builtins in registration order, with web-only commands appended", () => {
    ...
    // Web-only commands (currently `/compact`) are appended after the
    // TUI-mirrored block so the mirror invariant stays easy to audit.
    //
    // Web-only additions:
    //   - /compact (renamed from /compress) — force-fires the L2
    //     auto-compactor against the current session via
    //     POST /api/v1/sessions/{id}/compact. The TUI does not yet
    //     expose this command; when it does, this entry moves up into
    //     the TUI-mirrored block above.
