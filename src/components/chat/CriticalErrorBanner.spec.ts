...
describe("CriticalErrorBanner", () => {
  ...
  it("does not render when no critical error is in store state", () => {
    ...
    expect(wrapper.find('[data-testid="critical-error-banner"]').exists()).toBe(
      false,
    );
  });
  ...
  it("renders the banner with the wire-sanitized message when criticalError is set", () => {
    ...
    expect(
      wrapper.find('[data-testid="critical-error-message"]').text(),
    ).toContain("critical stream error");
  });
  ...
  it('exposes role="alert" for screen-reader announcement', () => {
    ...
    expect(
      wrapper.find('[data-testid="critical-error-banner"]').attributes("role"),
    ).toBe("alert");
  });
  ...
  it('hides the correlation id by default and reveals it via the "Show details" affordance', async () => {
    ...
    // they ask for it.
    expect(
      wrapper.find('[data-testid="critical-error-correlation-id"]').exists(),
    ).toBe(false);

    await wrapper
      .find('[data-testid="critical-error-details-toggle"]')
      .trigger("click");
  ...
  it("clears the banner from the DOM when the Dismiss button is clicked", async () => {
    ...
    await wrapper
      .find('[data-testid="critical-error-dismiss"]')
      .trigger("click");
  ...
  it("re-shows the banner after a dismiss when a fresh critical error arrives", async () => {
    ...
    await wrapper
      ...
      .trigger("click");
    expect(
      wrapper.find('[data-testid="critical-error-correlation-id"]').text(),
    ).toBe("second-id");
  });
  ...
  it('omits the "Show details" affordance when no correlation id is provided', () => {
    ...
    expect(
      wrapper.find('[data-testid="critical-error-details-toggle"]').exists(),
    ).toBe(false);
  });
});
