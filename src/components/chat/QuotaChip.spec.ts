...
describe('QuotaChip', () => {
  ...
  describe('visibility gating', () => {
    it('does not render when no provider/model is selected', async () => {
      const wrapper = mount(QuotaChip)
      await flushPromises()
      expect(wrapper.find('[data-testid="provider-quota-chip"]').exists()).toBe(false)
    })
    ...
    it('renders the chip after the first provider_quota event arrives', async () => {
      ...
      expect(wrapper.find('[data-testid="provider-quota-chip"]').exists()).toBe(true)
    })
  })

  describe('RateLimit variant', () => {
    function rateLimitEvent(percentRemaining: number, resetIso: string): SSEProviderQuotaEvent {
      return {
        ...baseEvent(),
        variant: 'rate_limit',
        rateLimit: {
    ...
    it('renders "{N}% remaining · resets Nm" with minute-formatted reset', async () => {
      ...
      const label = wrapper.find('[data-testid="provider-quota-rate-limit-label"]')
    ...
    it('formats short resets in seconds when under 60s', async () => {
      ...
      expect(wrapper.find('[data-testid="provider-quota-rate-limit-label"]').text())
    ...
    it('applies neutral severity at 42% remaining (>= 20%)', async () => {
      const wrapper = await mountWithEvent(rateLimitEvent(42, '2026-05-13T12:03:00Z'))
      expect(wrapper.find('[data-testid="provider-quota-chip"]').attributes('data-severity'))
        .toBe('neutral')
    })

    it('applies warning severity at 10% remaining (5-20% band)', async () => {
      const wrapper = await mountWithEvent(rateLimitEvent(10, '2026-05-13T12:03:00Z'))
      expect(wrapper.find('[data-testid="provider-quota-chip"]').attributes('data-severity'))
        .toBe('warning')
    })

    it('applies danger severity at 2% remaining (< 5%)', async () => {
      const wrapper = await mountWithEvent(rateLimitEvent(2, '2026-05-13T12:03:00Z'))
      expect(wrapper.find('[data-testid="provider-quota-chip"]').attributes('data-severity'))
        .toBe('danger')
    })
  ...
  describe('TokenSpend variant', () => {
    ...
    it('renders "$X.XX / $Y.YY" for the capped USD case', async () => {
      ...
      expect(wrapper.find('[data-testid="provider-quota-token-spend-label"]').text())
        .toBe('$2.41 / $50.00')
    })
    ...
    it('renders the thin bar when capped', async () => {
      ...
      const bar = wrapper.find('[data-testid="provider-quota-token-spend-bar"]')
    ...
    it('omits the bar when uncapped', async () => {
      const wrapper = await mountWithEvent(tokenSpendEvent(241, 0))
      expect(wrapper.find('[data-testid="provider-quota-token-spend-bar"]').exists()).toBe(false)
    })
    ...
    it('emits an "open" event when the chip is clicked (token_spend variant)', async () => {
      const wrapper = await mountWithEvent(tokenSpendEvent(241, 5000))
      await wrapper.find('[data-testid="provider-quota-chip"]').trigger('click')
  ...
  describe('NotConfigured variant', () => {
    ...
    it('renders "—" placeholder with neutral severity', async () => {
      const wrapper = await mountWithEvent(notConfiguredEvent('subscription-only'))
      const label = wrapper.find('[data-testid="provider-quota-not-configured-label"]')
      expect(label.exists()).toBe(true)
    ...
    it('surfaces the Reason verbatim on the data-reason attribute for assistive tech', async () => {
      const wrapper = await mountWithEvent(notConfiguredEvent('unknown-model:claude-experimental'))
      const label = wrapper.find('[data-testid="provider-quota-not-configured-label"]')
      expect(label.attributes('data-reason')).toBe('unknown-model:claude-experimental')
    })
  ...
  describe('tooltip', () => {
    ...
    it('surfaces "shared across instances" copy when StoreBackend=redis', async () => {
      ...
      const chip = wrapper.find('[data-testid="provider-quota-chip"]')
      expect(chip.attributes('title')).toContain('Shared across all FlowState instances')
    })
