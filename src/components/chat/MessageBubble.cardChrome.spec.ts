...
function makeRouter() {
  return createRouter({
    ...
    routes: [{ path: '/', name: 'home', component: defineComponent({ render: () => h('div') }) }],
  })
}
...
describe('MessageBubble — single card chrome per tool call', () => {
  ...
  const cases: Array<{ tool: string; toolInput?: string; body: string }> = [
    { tool: 'bash', toolInput: JSON.stringify({ command: 'ls' }), body: 'a\nb' },
    { tool: 'read', toolInput: JSON.stringify({ filePath: 'foo.txt' }), body: 'contents' },
    { tool: 'write', toolInput: JSON.stringify({ filePath: 'bar.txt' }), body: 'wrote it' },
    { tool: 'edit', toolInput: JSON.stringify({ filePath: 'baz.txt' }), body: '-old\n+new' },
    { tool: 'grep', toolInput: JSON.stringify({ pattern: 'foo' }), body: 'match-1' },
    { tool: 'glob', toolInput: JSON.stringify({ pattern: '*.ts' }), body: 'a.ts\nb.ts' },
