...
describe("AgentPicker", () => {
  ...
  it('shows "Select agent" when no agent is selected', async () => {
    ...
    expect(wrapper.find('[data-testid="agent-picker"]').text()).toContain(
      "Select agent",
    );
  });
  ...
  it("maps agent descriptions to fuzzy search item meta", async () => {
    ...
    expect(
      wrapper.find('[data-testid="fuzzy-search-item-planner"]').text(),
    ).toContain("Plans tasks");
    expect(
      wrapper.find('[data-testid="fuzzy-search-item-coder"]').text(),
    ).toContain("Writes code");
    wrapper.unmount();
  });
});
