...
describe("BashTool", () => {
  ...
  it("renders the bash command and output blocks", () => {
    ...
    expect(wrapper.get('[data-component="bash-command"]').text()).toContain(
      "ls -la",
    );
    expect(wrapper.get('[data-component="bash-output"]').text()).toContain(
      "file-a",
    );
  ...
  describe("render-cap", () => {
    function buildBody(
      lineCount: number,
      lineContent: (i: number) => string = (i) => `line-${i}`,
    ): string {
      const lines: string[] = [];
