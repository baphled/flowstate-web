...
describe('ToolErrorCard', () => {
  it('renders the error shell with the expected attributes', () => {
    ...
    expect(wrapper.find('[data-component="tool-error-details"]').exists()).toBe(false)
  })
  ...
  it('reveals error details when toggled open', async () => {
    ...
    expect(wrapper.get('[data-component="tool-error-details"]').text()).toContain('No such file or directory')
