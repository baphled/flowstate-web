...
describe("ProviderQuotaPanel", () => {
  ...
  describe("rate_limit branch", () => {
    it("renders the four windows + tightest summary", () => {
      const wrapper = mount(ProviderQuotaPanel, {
        props: { entry: rateLimitEntry },
      });

      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-rate-limit"]')
          .exists(),
      ).toBe(true);
      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-window-requests"]')
          .text(),
      ).toContain("100");
      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-window-tokens"]')
          .text(),
      ).toContain("100000");
      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-window-input"]')
          .text(),
      ).toContain("50000");
      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-window-output"]')
          .text(),
      ).toContain("50000");
      expect(
        wrapper.find('[data-testid="provider-quota-panel-tightest"]').text(),
      ).toBe("12%");
    });
    ...
    it("does not render the token_spend reset button", () => {
      ...
      expect(
        wrapper.find('[data-testid="provider-quota-panel-reset"]').exists(),
      ).toBe(false);
    });
  });
  ...
  describe("token_spend branch", () => {
    it("renders native + USD + cap + period + thresholds + pricing source + observed-at + estimator drift column", () => {
      const wrapper = mount(ProviderQuotaPanel, {
        props: { entry: tokenSpendEntry },
      });

      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-token-spend"]')
          .exists(),
      ).toBe(true);
      // Native = $12.34
      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-spent-native"]')
          .text(),
      ).toBe("$12.34");
      // USD equivalent (OD-6).
      expect(
        wrapper.find('[data-testid="provider-quota-panel-spent-usd"]').text(),
      ).toBe("$12.34");
      // Cap = $100.00
      expect(
        wrapper.find('[data-testid="provider-quota-panel-cap"]').text(),
      ).toBe("$100.00");
      expect(
        wrapper.find('[data-testid="provider-quota-panel-period"]').text(),
      ).toBe("month");
      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-threshold-amber"]')
          .text(),
      ).toBe("80%");
      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-threshold-red"]')
          .text(),
      ).toBe("95%");
      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-pricing-source"]')
          .text(),
      ).toBe("embedded");
      ...
      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-estimator-drift"]')
          .exists(),
      ).toBe(true);
    });
    ...
    it("disables the reset button when uncapped", () => {
      const wrapper = mount(ProviderQuotaPanel, {
        props: { entry: uncappedTokenSpendEntry },
      });
      const btn = wrapper.find('[data-testid="provider-quota-panel-reset"]');
      expect(btn.exists()).toBe(true);
  ...
  describe("not_configured branch", () => {
    it("renders the reason verbatim", () => {
      const wrapper = mount(ProviderQuotaPanel, {
        props: { entry: notConfiguredEntry },
      });
      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-not-configured"]')
          .exists(),
      ).toBe(true);
      expect(
        wrapper.find('[data-testid="provider-quota-panel-reason"]').text(),
      ).toBe("local provider exposes no quota signal");
    });
  ...
  describe("reset flow", () => {
    ...
    it("opens a confirm modal when reset is clicked", async () => {
      ...
      expect(
        wrapper
          .find('[data-testid="provider-quota-panel-reset-confirm"]')
          .exists(),
      ).toBe(false);

      await wrapper
        .find('[data-testid="provider-quota-panel-reset"]')
        .trigger("click");
      await flushPromises();
  ...
  describe("close behaviour", () => {
    it("emits close when the close button is clicked", async () => {
      ...
      await wrapper
        .find('[data-testid="provider-quota-panel-close"]')
        .trigger("click");
      expect(wrapper.emitted("close")).toHaveLength(1);
    });
    ...
    it("emits close when the backdrop is clicked", async () => {
      ...
      await wrapper
        .find('[data-testid="provider-quota-panel-backdrop"]')
        .trigger("click");
      expect(wrapper.emitted("close")).toHaveLength(1);
    });
  });
