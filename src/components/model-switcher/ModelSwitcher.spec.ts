...
describe('ModelSwitcher', () => {
  ...
  beforeEach(() => {
    ...
    listModelsSpy = vi.spyOn(api, 'listModels').mockResolvedValue(makeModelsResponse())
  })
  ...
  it('calls api.listModels on mount and caches the result', async () => {
    ...
    await wrapper.find('[data-testid="model-switcher-trigger"]').trigger('click')
    await wrapper.find('[data-testid="model-switcher-trigger"]').trigger('click')
    await flushPromises()
  ...
  it('shows the placeholder text when no model is selected', async () => {
    ...
    expect(wrapper.find('[data-testid="model-switcher-trigger"]').text()).toContain('Select model')
  })
  ...
  it('opens the dropdown listing providers and their models on click', async () => {
    ...
    expect(wrapper.find('[data-testid="model-switcher-dropdown"]').exists()).toBe(false)
  ...
  it('closes the dropdown when clicking outside', async () => {
    ...
    await flushPromises()
    await wrapper.find('[data-testid="model-switcher-trigger"]').trigger('click')
    expect(wrapper.find('[data-testid="model-switcher-dropdown"]').exists()).toBe(true)
  ...
  it('calls chatStore.setModel with split provider and model ids when an option is selected', async () => {
    ...
    const option = wrapper.find('[data-testid="model-option-anthropic-claude-opus-4"]')
  ...
  it('shows a loading state while fetching models', async () => {
    ...
    expect(wrapper.find('[data-testid="model-switcher-loading"]').exists()).toBe(true)

    resolveFn(makeModelsResponse())
    await flushPromises()

    expect(wrapper.find('[data-testid="model-switcher-loading"]').exists()).toBe(false)
  })
  ...
  it('shows an error state with a retry button when listModels fails, and retries on click', async () => {
    ...
    expect(wrapper.find('[data-testid="model-switcher-error"]').exists()).toBe(false)
  ...
  it('reactively updates the trigger label when chatStore current selection changes', async () => {
    ...
    expect(wrapper.find('[data-testid="model-switcher-trigger"]').text()).toContain('openai/gpt-4o')
  })
})
