...
describe('KeyboardHelpModal', () => {
  ...
  it('renders the modal panel when open=true', () => {
    ...
    expect(wrapper.find('[data-testid="keyboard-help-group-composer"]').exists()).toBe(true)
    expect(wrapper.find('[data-testid="keyboard-help-group-streaming-control"]').exists()).toBe(true)
  })
  ...
  it('emits close on the backdrop click', async () => {
    const wrapper = mount(KeyboardHelpModal, { props: { open: true } })
    await wrapper.get('[data-testid="keyboard-help-backdrop"]').trigger('click')
    expect(wrapper.emitted('close')).toBeTruthy()
  })

  it('emits close on Escape keypress while open', async () => {
    const wrapper = mount(KeyboardHelpModal, { props: { open: true }, attachTo: document.body })
    await flushPromises()
