...
describe("Notifications evidence harness (writes /tmp/notifications-evidence/*.html)", () => {
  ...
  it('captures: 01 single tool_call → "Running command" toast', async () => {
    ...
    store.applyContentEvent(
      JSON.stringify({ type: "tool_call", name: "bash", status: "running" }),
    );
    await flushPromises();
  ...
  it('captures: 02 multi-tool burst → aggregated "+ N more" toast', async () => {
    ...
    const store = useChatStore();
    store.applyContentEvent(
      JSON.stringify({ type: "tool_call", name: "Read", status: "running" }),
    );
    store.applyContentEvent(
      JSON.stringify({ type: "tool_call", name: "Grep", status: "running" }),
    );
    store.applyContentEvent(
      JSON.stringify({ type: "tool_call", name: "Edit", status: "running" }),
    );
    store.applyContentEvent(
      JSON.stringify({ type: "tool_call", name: "Bash", status: "running" }),
    );
    store.applyContentEvent(
      JSON.stringify({ type: "tool_call", name: "Write", status: "running" }),
    );
    await flushPromises();
  ...
  it("captures: 07 friendly tool labels for all common tools", async () => {
    ...
    for (const sample of samples) {
      ...
      frames.push(
        `<section><h2 style="font-size:0.85rem;color:#aaa">${sample.name} → ${sample.expected}</h2>${frameHtml}</section>`,
      );
